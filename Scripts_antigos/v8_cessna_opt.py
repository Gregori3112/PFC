...
# === LIMITES DAS VARIÁVEIS (coerente com PSO) ===
SWEEP_MIN, SWEEP_MAX = 0.0, 30.0       # [graus]
TWIST_MIN, TWIST_MAX = -6.0, 6.0       # [graus]
CTIP_MIN,  CTIP_MAX  = 1.0, 4.0        # [ft]
ALPHA_MIN, ALPHA_MAX = -4.0, 4.0       # [graus]
CROOT_FT = 4.76339                     # corda fixa da raiz [ft]
S_REF_FT2 = 172.71                     # área total da asa [ft²]
...
def FCN(x: np.ndarray, return_LD_only=False) -> float:
    ...
    try:
        ...
        ctip_ft   = np.clip(ctip_ft, CTIP_MIN, CTIP_MAX)
        sweep_deg = np.clip(sweep_deg, SWEEP_MIN, SWEEP_MAX)
        twist_deg = np.clip(twist_deg, TWIST_MIN, TWIST_MAX)

        # --- Geometria básica ---
        Croot_ft = 4.0  # corda da raiz (ft)
        S_ref_ft2 = 172.71  # área total da asa (ft²)

        # Limita a corda da ponta
        ctip_ft = np.clip(ctip_ft, 1.0, Croot_ft)

        # Derivações
        taper = ctip_ft / Croot_ft
        b_total_ft = (2 * S_ref_ft2) / (Croot_ft + ctip_ft)
        ...
        with suppress_all_output():
            ...
            # --- AJUSTE DE GEOMETRIA (forçando cordas) ---
            # 1) Span que mantém área constante para Croot e Ctip definidos
            vsp.SetParmVal(wing_id, "Span", "XSec_1", b_total_ft / 2.0)

            # 2) Força cordas diretamente na seção (não use "Taper")
            #    Observação: estes nomes de parâmetros são os usados pelo WingGeom
            #    nas seções do plano (XSec_1). Funcionam em 3.45.x.
            vsp.SetParmVal(wing_id, "Root_Chord", "XSec_1", Croot_ft)
            vsp.SetParmVal(wing_id, "Tip_Chord",  "XSec_1", ctip_ft)

            # (Opcional, caso o arquivo original tenha ties/eq-par): repetir o span
            # após cordas; em alguns modelos o EqPar pode sobrescrever algo.
            vsp.SetParmVal(wing_id, "Span", "XSec_1", b_total_ft / 2.0)
    ...
    print(f"[debug] Sweep={sweep_deg:.2f}, Twist={twist_deg:.2f}, Ct={ctip_ft:.2f}, Taper={taper:.3f}, Span={b_total_ft:.2f}, Alpha={alpha_deg:.2f}")



# === Teste local ===
if __name__ == "__main__":
    # sweep_deg, twist_deg, ctip_ft, alpha_deg
    x0 = np.array([20, 0, 2.0, 2.0])
