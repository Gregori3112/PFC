...
import v5_cessna_opt
importlib.reload(v5_cessna_opt)
from v5_cessna_opt import FCN
...
while not flag:
    ...
    for i in range(pop):
        ...
        try:
            ...
            ynew = 1e6  # aplica penalidade

            # --- Armazena o L/D (penalidade = NaN para não distorcer o gráfico)
    ...
    if k >= itermax or (k > 5 and abs(np.mean(history["L_over_D"][-5:]) - history["L_over_D"][-1]) < 1e-3):
        flag = True
